...
import matplotlib
matplotlib.use('Agg')  # 서버 환경에서 GUI 백엔드 사용 방지
...
from kepler_utils import generate_walker_delta_constellation, generate_tle_from_kepler

import logging

# ----------------------------------------------------
# 로거 설정 (스크립트 최상단에 한 번만)
# ----------------------------------------------------
...
def plot_isl_total_minutes_histogram(isl_details, bins=30):
    """
    ISL 가용 시간(total_minutes)의 분포를 히스토그램으로 그립니다.
    """
    total_minutes = [d['total_minutes'] for d in isl_details]
    
    plt.figure()
    plt.hist(total_minutes, bins=bins)
    plt.xlabel('Total Available Minutes')
    plt.ylabel('Number of Satellite Pairs')
    plt.title('Distribution of ISL Availability Across All Pairs')
    plt.tight_layout()
    # output_path가 속한 디렉터리 자동 생성
    dpi = 300
    output_path = './plt_dir/Distribution of ISL Availability Across All Pairs.png'
    out_dir = os.path.dirname(output_path)
    if out_dir and not os.path.exists(out_dir):
        os.makedirs(out_dir, exist_ok=True)
    plt.savefig(output_path, dpi=dpi)
    
def plot_isl_timeline_for_pair(detail):
    """
    특정 위성 쌍(detail)의 ISL 통신 창(window)들을 타임라인(barh)으로 시각화합니다.
    """
    windows = detail['windows']
    # broken_barh에 넣기 위해 (시작_숫자, 기간_일 단위) 튜플 리스트 생성
    segments = [
        (
            mdates.date2num(start), 
            duration_min / 1440.0
        )
        for start, _, duration_min in windows
    ]

    fig, ax = plt.subplots()
    ax.broken_barh(segments, (0, 5))
    ax.xaxis_date()
    ax.xaxis.set_major_formatter(mdates.DateFormatter('%Y-%m-%d\n%H:%M'))
    ax.set_yticks([])
    sat1, sat2 = detail['pair']
    ax.set_title(f'ISL Windows for Sat{sat1}-Sat{sat2}')
    ax.set_xlabel('Time (UTC)')
    ax.set_ylabel('ISL Window')
    plt.tight_layout()
    # output_path가 속한 디렉터리 자동 생성
    dpi = 300
    output_path = './plt_dir/ISL Windows for Sat pair.png'
    out_dir = os.path.dirname(output_path)
    if out_dir and not os.path.exists(out_dir):
        os.makedirs(out_dir, exist_ok=True)
    plt.savefig(output_path, dpi=dpi)
...
# ----------------------------------------------------
# 1) Earth blockage (LOS) 검사 함수
# ----------------------------------------------------
def is_line_of_sight(r1: np.ndarray,
                     r2: np.ndarray,
                     earth_radius: float = 6371.0) -> bool:
    """
    두 위성 위치 벡터 r1, r2 사이의 직선 경로가 지구(구체)에 의해 차폐되지 않는지 검사.
    선분 p(t)=r1 + t*(r2-r1)에 대해 지구 구면(x^2+y^2+z^2=earth_radius^2)과 교차 여부를 판별.
    """
    d = r2 - r1
    a = np.dot(d, d)
    b = 2 * np.dot(r1, d)
    c = np.dot(r1, r1) - earth_radius**2

    disc = b*b - 4*a*c
    if disc <= 0:
        # 판별식 ≤0 이면 실근 없음 → 차폐 없이 통과
        return True

    sqrt_disc = np.sqrt(disc)
    t1 = (-b + sqrt_disc) / (2*a)
    t2 = (-b - sqrt_disc) / (2*a)
    # 두 교차점 모두 선분 범위 밖(t<0 또는 t>1)이면 차폐 없음
...
# 2) ISL 통신 창 계산 함수
# ----------------------------------------------------
def compute_isl_windows(constellation_results: list,
                        threshold_range_km: float,
                        earth_radius: float = 6371.0,
                        step_minutes: int = 1) -> list:
    """
    모든 위성 쌍에 대해 ISL 통신 가능 여부를 시간별로 체크하고,
    연속된 통신 창(window)을 찾아 반환.
    
    Returns: [
      {
        "pair": (sat_id1, sat_id2),
        "windows": [(start_dt, end_dt, duration_min), ...],
        "total_minutes": 총통신시간
      }, ...
    ]
    """
    isl_details = []
    N = len(constellation_results)
    total_pairs = N*(N-1)//2
    processed = 0
    logger.info(f"총 {N}개 위성, {total_pairs}개 페어에 대해 ISL 계산 시작 (range ≤ {threshold_range_km} km)")

    for i in range(N):
        sat_i = constellation_results[i]
        t = sat_i["times"]
        r_i = sat_i["coords"]

        for j in range(i+1, N):
            sat_j = constellation_results[j]
            r_j = sat_j["coords"]
            processed += 1

            # 진행 상황 로깅 (매 500 페어마다)
            if processed % 500 == 0 or processed == total_pairs:
                logger.info(f"[{processed}/{total_pairs}] 페어 검사 중: Sat{sat_i['sat_id']}–Sat{sat_j['sat_id']}")

            valid = []
            for v1, v2 in zip(r_i, r_j):
                dist = np.linalg.norm(v1 - v2)
                if dist <= threshold_range_km and is_line_of_sight(v1, v2, earth_radius):
                    valid.append(True)
                else:
                    valid.append(False)

            windows = get_contiguous_windows(t, valid, step_minutes=step_minutes)
            total = sum(w[2] for w in windows)

            # 이 페어 결과 로깅
            logger.debug(f"  Sat{sat_i['sat_id']}-Sat{sat_j['sat_id']} → 윈도우 {len(windows)}개, 총 {total:.1f}분")

            isl_details.append({
                "pair": (sat_i["sat_id"], sat_j["sat_id"]),
                "windows": windows,
                "total_minutes": total
            })

    logger.info("ISL 계산 완료")
    return isl_details

# ----------------------------------------------------
# 3) CSV로 저장하는 함수
# ----------------------------------------------------
def save_isl_csv(isl_details: list, filename: str = "isl_windows.csv"):
    """
    sat1_id, sat2_id, window_start, window_end, duration_min
    """
    logger.info(f"ISL 윈도우를 CSV로 저장: {filename}")
    with open(filename, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["sat1_id", "sat2_id", "window_start", "window_end", "duration_min"])
        for detail in isl_details:
            s1, s2 = detail["pair"]
            for start, end, dur in detail["windows"]:
                writer.writerow([s1, s2, start.isoformat(), end.isoformat(), f"{dur:.2f}"])
    print(f"ISL windows saved to '{filename}'")
...
def plot_constellation_3d(
    ...
    # output_path가 속한 디렉터리 자동 생성
    dpi = 300
    output_path = './plt_dir/Walker Delta Constellation 3D.png'
    out_dir = os.path.dirname(output_path)
    if out_dir and not os.path.exists(out_dir):
        os.makedirs(out_dir, exist_ok=True)
    plt.savefig(output_path, dpi=dpi)
...
def plot_elevation_vs_time(constellation_results, observer, title="Elevation vs Time (Berlin)"):
    ...
    # output_path가 속한 디렉터리 자동 생성
    dpi = 300
    output_path = './plt_dir/Elevation vs Time (Berlin).png'
    out_dir = os.path.dirname(output_path)
    if out_dir and not os.path.exists(out_dir):
        os.makedirs(out_dir, exist_ok=True)
    plt.savefig(output_path, dpi=dpi)
...
if __name__ == "__main__":
    ...
    # 예: ISL 최대 거리 1000 km 조건
    threshold_range_km = 1000.0  
    isl_details = compute_isl_windows(
        constellation_results,
        threshold_range_km=threshold_range_km,
        earth_radius=6371.0,
        step_minutes=step_minutes
    )
    save_isl_csv(isl_details, filename="isl_windows.csv")
